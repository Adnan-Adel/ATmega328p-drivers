...
std_return_type_t seg_init(const seg_t* _seg_obj)
{
	...
	else
	{
		...
		{
			ret_val= gpio_pin_set_direction(_seg_obj->seg_pins[index_i]._port, _seg_obj->seg_pins[index_i]._pin, OUTPUT);
		}
...
std_return_type_t seg_write_nnum(const seg_t* _seg_obj, uint8_t num)
{
	...
	else
	{
		...
		if(_seg_obj->seg_mode == SEG_COM_ANODE)
		{
			...
			{
				ret_val= gpio_pin_set_value(_seg_obj->seg_pins[index_i]._port, _seg_obj->seg_pins[index_i]._pin, (com_anode_arr[num] >> index_i)&0x01);
			}
		}
